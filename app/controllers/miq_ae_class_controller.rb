  ...
  def set_root_node
    ...
    case id[0]
      ...
      when "aen"
        ...
        rec = Tenant.current_tenant.visible_domains
        @record = nil
...
private
  ...
  def get_rec_name(rec)
    ...
    if rec.kind_of?(MiqAeNamespace) && rec.domain?
      editable_domain = Tenant.current_tenant.editable_domains.include?(rec)
  ...
  def ordered_domains_for_priority_edit_screen
    Tenant.current_tenant.editable_domains
      .collect { |d| d.editable? ? d.name : add_read_only_suffix(d.name,
                                                                 Tenant.current_tenant.editable_domains.include?(d),
                                                                 rec.enabled
                                                                )
               }
  end
