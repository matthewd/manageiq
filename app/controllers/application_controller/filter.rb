...
module ApplicationController::Filter
      ...
      when "find"
        if params[:chosen_field] && params[:chosen_field] != edit_expkey[:exp_field] # Did the field change?
          ...
          unless params[:chosen_field] == "<Choose>"
            ...
            edit_expkey[:exp_available_cfields] = []      # Create the check fields pulldown array
  ...
  def load_default_search(id)
    ...
    edit_expkey = @edit[@expkey]
    edit_expkey[:expression] = []                           # Store exps in an array
    @edit[:new] = {}
    @edit[:new][@expkey] = edit_expkey[:expression]                # Copy to new exp
  ...
  def exp_commit(token)
    ...
    when "field"
      if edit_expkey[:exp_field].nil?
        ...
        exp[edit_expkey[:exp_key]] = {}                        # Add in the new key
    ...
    when "find"
      if edit_expkey[:exp_field].nil?
        ...
        exp.delete(edit_expkey[:exp_orig_key])                     # Remove the old exp fields
        exp[edit_expkey[:exp_key]] = {}                        # Add in the new key
        exp[edit_expkey[:exp_key]]["search"] = {}              # Create the search hash
        skey = edit_expkey[:exp_skey]
        exp[edit_expkey[:exp_key]]["search"][skey] = {}        # Create the search operator hash
        ...
        exp[edit_expkey[:exp_key]][chk] = {}                 # Create the check hash
        ckey = edit_expkey[:exp_ckey]
        exp[edit_expkey[:exp_key]][chk][ckey] = {}           # Create the check operator hash
  ...
  def process_datetime_expression_field(value_key, exp_key, exp_value_key)
    ...
    if [:date, :datetime].include?(edit_expkey[value_key][:type])  # Set value for date/time fields
      ...
      if edit_expkey[exp_key] == EXP_FROM
        ...
        edit_expkey[exp_value_key] = edit_expkey[value_key][:date_format] == "s" ? [] : [EXP_TODAY]
      end
    end
  end
